...
Single Amazon EKS cluster with Graviton-based managed nodes for system workloads and Karpenter for elastic tenant capacity.
...
StorageClasses for tenant PVCs (Amazon EFS is default):
...
## ArgoCD (Amazon EKS Capability)

ArgoCD installed via Helm (scripts/setup-argocd.sh). Manages tenant resources via Application CRs generated by the ApplicationSet.
