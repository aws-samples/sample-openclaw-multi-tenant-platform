...
`OpenClawTenantRole` -- assumed by `pods.eks.amazonaws.com` with `sts:TagSession`. The role name is auto-generated by AWS CDK (e.g., `OpenClawEksStack-TenantRole75C06F3F-xxxxx`).
...
Created by PostConfirmation AWS Lambda during tenant provisioning. Maps namespace `openclaw-{tenant}` + SA `{tenant}` to the shared tenant role (ARN from AWS CDK stack output `TenantRoleArn`).
