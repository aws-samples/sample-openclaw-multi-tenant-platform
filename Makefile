...
check-sensitive:
	@FOUND=$$(grep -rn '123456789012\|AKIA' --include='*.ts' --include='*.py' --include='*.md' --include='*.sh' . | grep -v node_modules | grep -v cdk.out | grep -v cdk.json | grep -v '/test/' | grep -v 'test_' | grep -v '.test.' | grep -v 'AGENTS.md' | grep -v 'install-hooks.sh' | grep -v 'generate-config.sh' | grep -v 'Example:' | grep -v 'placeholder' || true); \
